...
fn display_service(i2c: i2c::I2cDriver, rs: Arc<Mutex<RemoteState>>) -> Result<()> {
...
fn statistics_service(rs: Arc<Mutex<RemoteState>>) {
    let app_config = CONFIG;
    loop {
        if let Ok(mut stream) = TcpStream::connect(format!("{:}:9999", app_config.target_ip)) {
            match rs.lock() {
                Ok(mut state) => match state.mode {
                    Mode::Monitor => {
                        state.monitor.stats = kasa_protocol::get_realtime_by_idx(
                            &mut stream,
...
fn main() -> Result<()> {
    ...
    let mut rs = Arc::new(Mutex::new(RemoteState::new()));
    ...
    let _d_thread = thread::Builder::new().stack_size(10000).spawn(move || {
        let _ = statistics_service(rs_stat);
    });
    ...
    let _d_thread = thread::Builder::new().stack_size(10000).spawn(move || {
        let _ = display_service(i2c, rs_disp);
    });
    ...
    let _e_thread = thread::Builder::new().stack_size(3000).spawn(move || {
        let _ = encoder_service(enc_a.into(), enc_b.into(), rs_enc);
    });
